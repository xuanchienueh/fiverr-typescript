...
export default function UpdateInfoUser(props: { setshowModal: any }) {
  const [form] = Form.useForm();
  let setshowModal = props.setshowModal;
  const dispatch = useDispatch();
  const [getInfoDetailUser, setGetInfoDetailUser] = useState(NaN);
    ...
    if (getInfoDetailUser && infoUserLogin.token) {
      manageUserService
        ...
        .then((result: any) => {
          console.log(result);
          dispatch(
            userAction.getInfoUserLogin({ user: { ...result.data }, token: infoUserLogin.token })
          );
        })
        .catch((err) => console.log(err));
    }

    return () => setGetInfoDetailUser(0);
  }, [getInfoDetailUser]);
  ...
  const onFinish = (values: any) => {
    ...
    let result = manageUserService.updateInfoUserService(infoUserLogin.user._id, values);
