...
import { userAction } from "store/manageUser";
import { RootState } from "store";
import { getlistServiceUserBoughtAction } from "store/manageJobs/manageJob.action";

export default function Profile() {
  const dispatch = useDispatch();
  const { infoUserLogin } = useSelector((state: RootState) => state.manageUserReducer);
  const { listServiceUserBought } = useSelector((state: RootState) => state.manageJobReducer);
  const [showUploadAvatar, setShowUploadAvatar] = useState(false);
  const [showModal, setshowModal] = useState(false);
  let [avatar, setAvatar] = useState(infoUserLogin.user.avatar);
  const inputUpload: { current: any } = useRef();
  ...
  const handleChangeImage = async (e: any) => {
    ...
    try {
      let result: any = await manageUserService.uploadAvatarService(data);
      setAvatar(result.data.avatar);
      dispatch(
        userAction.getInfoUserLogin({
          ...infoUserLogin,
  ...
  useEffect(() => {
    dispatch(getlistServiceUserBoughtAction());
  }, []);
      ...
      <div className="container max-widthContainer">
        <div className="row">
          ...
          <div className="con-12 col-lg-8">
            ...
            {listServiceUserBought?.map((service: any, key: number) => (
